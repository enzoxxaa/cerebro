...
$\pi$ sale de las \textbf{tasas locales} $T_{ij}$, no de cuántas veces se visitó cada microestado. Sembrar más en una región mejora la \emph{precisión} con la que se estima $T_{ij}$ ahí --- no cambia su valor esperado, siempre que haya suficientes transiciones observadas para estimarlo. Esto es, formalmente, lo que resuelve el problema de la \S\ref{sec:precio}\slash{}Clase 1: $\hat F(s) = F(s) - RT\ln(w_i/\pi_i)$ --- el MSM reponde $w_i \to \pi_i$. Es la respuesta formal a la objeción ``pero sembraste con un sesgo''.
\end{resultado}
